    ...
    public Quaternion(Vec3d from, Vec3d to) {
        ...
        if ((fromSqNorm < epsilon) || (toSqNorm < epsilon)) {
            ...
            if (axisSqNorm < epsilon) {
                ...
                //setAxisAngle(axis, (double) Math.PI);
                axis = from.orthogonalVec();
                //}
                //return;
            }

            double angle = Math.asin(Math.sqrt((double) (axisSqNorm / (fromSqNorm * toSqNorm))));
            //double va = from.dot3(to.normalized());
            //angle = Math.acos(va);
            if (from.dot3(to.normalized()) < 0.0) {
                angle = Math.PI - angle;
            }
            setAxisAngle(axis, (double) angle);
        }
    }
    ...
    public void setFromRotationMatrix(Matrix3d mat) {
        ...
        if (onePlusdoublerace > 1E-5) {
            ...
            if ((m[0][0] > m[1][1]) & (m[0][0] > m[2][2])) {
                double s = (double) (java.lang.Math.sqrt(1.0 + m[0][0] - m[1][1] - m[2][2]) * 2.0);
                q[0] = 0.25f * s;
                q[1] = (m[0][1] + m[1][0]) / s;
                q[2] = (m[0][2] + m[2][0]) / s;
                q[3] = (m[1][2] - m[2][1]) / s;
            } else if (m[1][1] > m[2][2]) {
                double s = (double) (java.lang.Math.sqrt(1.0 + m[1][1] - m[0][0] - m[2][2]) * 2.0);
                q[0] = (m[0][1] + m[1][0]) / s;
                q[1] = 0.25f * s;
                q[2] = (m[1][2] + m[2][1]) / s;
                q[3] = (m[0][2] - m[2][0]) / s;
            } else {
                double s = (double) (java.lang.Math.sqrt(1.0 + m[2][2] - m[0][0] - m[1][1]) * 2.0);
                q[0] = -(m[0][2] + m[2][0]) / s;
                q[1] = -(m[1][2] + m[2][1]) / s;
                q[2] = -0.25f * s;
                q[3] = (m[0][1] - m[1][0]) / s;
...
//        Vec3f vn = new Vec3f(v);
    ...
    public Quaternion log() {
        ...
        if (len < 1E-6) {
            ...
            return new Quaternion(q[0] * coef, q[1] * coef, q[2] * coef, (double) 0.0);
        }
    }
    ...
    public Quaternion exp() {
        ...
        if (theta < 1E-6) {
            return new Quaternion(q[0], q[1], q[2], (double) java.lang.Math.cos(theta));
        } else {
            double coef = (double) java.lang.Math.sin(theta) / theta;
            return new Quaternion(q[0] * coef, q[1] * coef, q[2] * coef, (double) java.lang.Math.cos(theta));
        }
    }
    ...
    public Vec3d getEulerAngleXYZByAngle() {
        ...
        double roll = (double) Math.toDegrees(xyz.x());
        double pitch = (double) Math.toDegrees(xyz.y());
        double yaw = (double) Math.toDegrees(xyz.z());
        return new Vec3d(roll, pitch, yaw);
    }
    ...
    public void fromRotatedPlane(Vec3d a, Vec3d b, Vec3d a_original, Vec3d b_original) {
        ...
        if (a.equals(a_original)) {
            ...
            if (!n_original.equals(n)) {
                ...
                intersection.normalize();
                coplane.setAxisAngle(intersection, (double) java.lang.Math.acos(cosTheta));
            }
...
//        }
        ...
        if (axis_a.length() < 0.1) {
            ...
            q1.setAxisAngle(axis_a, (double) Math.acos(cosTheta_a));
        }
        ...
        if (axis_n.length() < 0.1) {
            ...
            q2.setAxisAngle(axis_n, (double) Math.acos(cosTheta_n));
            if (cosTheta_n > 0) {
                q3.setAxisAngle(axis_n, 3.1415926f - (double) Math.acos(cosTheta_n));
            } else {
                q3.setAxisAngle(axis_n, 3.1415926f + (double) Math.acos(cosTheta_n));
            }
        }
...
//            }
         ...
         * (double) Math.acos(cosTheta_n); q2.setAxisAngle(sign, angle);
         * q21.setAxisAngle(sign, (double) -angle); // } else { //
         * q2.setAxisAngle(axis_n, -(double) Math.acos(cosTheta_n)); // }
         *
         * q3.setAxisAngle(sign, 3.1415926f - (double) angle);
         *
         * q31.setAxisAngle(sign, 3.1415926f + (double) angle);
         *
         * q32.setAxisAngle(sign, -3.1415926f - (double) angle);
         *
         * q33.setAxisAngle(sign, -3.1415926f + (double) angle);
         *
         *
