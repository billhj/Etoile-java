...
public class DrawStuffGL extends LwJGL implements DrawStuffApi {
	...
	private static final float LIGHTZ = 0.4f;
	//#define LIGHTX (1.0f)
	//#define LIGHTY (0.4f)
	...
	private static final float ground_ofsz = 0.5f;
...
//	}
	...
	private float[] view_rxyz = new float[3];	// heading, pitch, roll (degrees)
	...
	{
		for (int i=0; i<3; i++) {
			while (view_rxyz[i] > 180) view_rxyz[i] -= 360;
			while (view_rxyz[i] < -180) view_rxyz[i] += 360;
		}
	}
...
//	static void drawSky (float view_xyz[3])
  ...
  GL11.glFogf (GL_FOG_END, 5.0);
		...
		GL11.glTexCoord2f (-gsize*ground_scale + ground_ofsx,
				gsize*ground_scale + ground_ofsz);
	...
	static {
		light_position.put(new float[] { LIGHTX, 1.0f, LIGHTZ, 0.0f }).flip();
...
//		memcpy (view2_hpr,view_hpr);//,sizeof(float)*3);
		...
		setCamera (view2_xyz[0],view2_xyz[1],view2_xyz[2],
				view2_rxyz[0],view2_rxyz[1],view2_rxyz[2]);
...
//				static GLfloat t_params[4] = {0.817f,-0.817f,0.817f,1};
	...
	static {
		...
		t_params_SSDM.put(new float[]{0,ground_scale,0,ground_ofsz}).flip();
	}
...
//			static GLfloat t_params[4] = {0,ground_scale,0,ground_ofsy};
	...
	@Override
	public void dsGetViewpoint (float[] xyz, float[] rxyz)
	{
